...
function gaScore(prior, current, entry, lobbyAvgTier, mastery) {
  ...
  return {
    ...
    otp: !otpDenied && (champCount >= 4 || mastery >= 150000),
...
export async function analyzeMatch(c, db, { name, tag, puuid, matchId }) {
  ...
  return {
    ...
    matchmaking: fair.verdict, direction: fair.direction, oneLiner: fair.oneLiner, verdictTooltip: fair.verdictTooltip, teamGA: fair.teamGA, duoBonus: fair.duoBonus, spreads: fair.spreads,
    duos: duos.map(d => [d.a, d.b, `${d.shared}/5 pre-game games together`, d.team === me.team ? 'ally' : 'enemy']),
    players: rows.map(r => ({ n: r.name, team: r.team === 100 ? 'blue' : 'red', rank: r.rank, pos: r.pos, champ: r.champ, kda: r.kda, dmg: r.dmg, cs: r.cs, cspm: +(r.cs / mins).toFixed(1), ga: r.ga.ga, form: `${r.ga.wins}W-${r.ga.n - r.ga.wins}L`, streak: r.ga.streak, wr: r.ga.wr, seasonGames: r.ga.seasonGames, duo: duoNames.has(r.name), duoWith: duoWith[r.name] || null, duoShared: duoShared[r.name] ?? null, duoRecord: duoRec[r.name] || null, deniedChamp: r.ga.deniedChamp || null, flags: [!r.ga.smurf && r.ga.autofill && 'autofill', !r.ga.smurf && r.ga.firstTime && 'first-time', r.ga.otp && 'otp', r.ga.otpDenied && 'otp-denied', !r.ga.smurf && r.ga.tilt && 'tilt', r.ga.rusty && 'rusty', r.ga.smurf && 'smurf', r.ga.afkRisk && 'afk-risk'].filter(Boolean), place: r.place || null, badge: r.badge || null })),
  };
}
...
export async function analyzeLive(c, db, { name, tag, puuid }) {
  ...
  const entry = {
    ...
    players: rows.map(r => ({ n: r.name, team: r.team === 100 ? 'blue' : 'red', rank: r.rank, pos: r.pos, champ: r.champ, ga: r.ga ? r.ga.ga : null, form: r.ga ? `${r.ga.wins}W-${r.ga.n - r.ga.wins}L` : null, streak: r.ga ? r.ga.streak : null, wr: r.ga ? r.ga.wr : null, seasonGames: r.ga ? r.ga.seasonGames : null, duo: duoNames.has(r.name), duoWith: duoWith[r.name] || null, duoShared: duoShared[r.name] ?? null, duoRecord: duoRec[r.name] || null, deniedChamp: r.ga ? (r.ga.deniedChamp || null) : null, flags: r.ga ? [!r.ga.smurf && r.ga.autofill && 'autofill', !r.ga.smurf && r.ga.firstTime && 'first-time', r.ga.otpDenied && 'otp-denied', !r.ga.smurf && r.ga.tilt && 'tilt', r.ga.rusty && 'rusty', r.ga.smurf && 'smurf', r.ga.afkRisk && 'afk-risk'].filter(Boolean) : [], place: null, badge: null, posInferred: true })),
  };
