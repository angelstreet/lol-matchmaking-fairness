...
function fairness(rows, duos, userTeam) {
  ...
  // v4.8: autofill asymmetry now gets its OWN explicit term instead of relying entirely on the
  ...
  // not sure how I can be in my favor with 2 autofill." +5 GA-equivalent per net autofill,
  // user-perspective (enemy autofills help the user, the user's own autofills hurt).
  const net = 0.5 * netLaneUserCapped + gaGapUser + 1.5 * duoDiff + 1.0 * (enemyAutofillCount - userAutofillCount) * 5;
...
export async function analyzeMatch(c, db, { name, tag, puuid, matchId }) {
  ...
  return {
    ...
    matchmaking: fair.verdict, direction: fair.direction, oneLiner: fair.oneLiner, verdictTooltip: fair.verdictTooltip, teamGA: fair.teamGA, duoBonus: fair.duoBonus, spreads: fair.spreads, autofillCounts: fair.autofillCounts, winProb: fair.winProb,
