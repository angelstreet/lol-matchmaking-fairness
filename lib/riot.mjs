...
export async function analyzeMatch(c, db, { name, tag, puuid, matchId }) {
  ...
  const teamKills = { 100: 0, 200: 0 }, teamDmg = { 100: 0, 200: 0 };
  for (const p of parts) { teamKills[p.teamId] += p.kills; teamDmg[p.teamId] += p.totalDamageDealtToChampions; }
  ...
  const scored = parts.map((p, idx) => {
    ...
    return { idx, s: kdaR * 1.5 + kp * 12 + dmgShare * 12 + cspm * 0.6 + vspm * 2 };
  }).sort((a, b) => b.s - a.s);
