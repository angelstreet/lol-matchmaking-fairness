...
function fairness(rows, duos, userTeam) {
  ...
  // are close. v4.2: a duo'd player also gets a lane bonus added — the team-level duo bonus above
  ...
  const laneAdj = ['TOP', 'JUNGLE', 'MIDDLE', 'BOTTOM', 'UTILITY'].map(pos => {
    ...
    if (!b || !r) return null;
    const bGa = b.ga.ga - riskOf(b) - counterPenalty(b.champ, r.champ) + (laneBonusOf[b.name] || 0);
    const rGa = r.ga.ga - riskOf(r) - counterPenalty(r.champ, b.champ) + (laneBonusOf[r.name] || 0);
