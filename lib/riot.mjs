...
function fairness(rows, duos, userTeam) {
  ...
  // champion pair), applied to both of that team's bot-lane rows below. v4.21: DRAFT, not MM — a
  // champion PAIRING choice, same family as the counter penalty below.
  ...
  const laneAdj = ['TOP', 'JUNGLE', 'MIDDLE', 'BOTTOM', 'UTILITY'].map(pos => {
    ...
    // penalty, no bot synergy — those are the DRAFT component below.
    ...
    // double-penalizing) + bot-lane synergy (BOTTOM/UTILITY only, each side's own capped value).
    ...
    let bSynergy = 0, rSynergy = 0;
    if (pos === 'BOTTOM' || pos === 'UTILITY') {
      if (blueBotSynergy) bSynergy = clampBotSynergy(blueBotSynergy.delta);
      if (redBotSynergy) rSynergy = clampBotSynergy(redBotSynergy.delta);
    }
  ...
  // synergy-driven bot lane either), plus its own modest, capped ±5 bot-synergy net term
  // (deliberately the smallest, most conservative term of either sub-net — "these two champs pair
  // well on paper" is a much weaker signal than an actual counter pick or GA gap).
  ...
  const netDraft = 0.5 * draftLaneUserCapped + botSynergyNetTerm;
  ...
  // thresholds (±5 vs MM's ±10) since netDraft's inputs (counter, bot synergy) are individually
  // capped much lower than netMM's. Tooltip names the actual fired draft components — a countered
  // lane by champ name, the bot-synergy net contribution — so "champ select, not matchmaking" is
  // never just asserted, it's shown.
  ...
  if (botSynergyNetTerm !== 0) draftReasons.push(`bot synergy ${botSynergyNetTerm >= 0 ? '+' : ''}${botSynergyNetTerm.toFixed(1)}`);
...
export async function analyzeMatch(c, db, { name, tag, puuid, matchId }) {
  ...
  return {
    ...
    matchmaking: fair.verdict, direction: fair.direction, oneLiner: fair.oneLiner, verdictTooltip: fair.verdictTooltip, teamGA: fair.teamGA, duoBonus: fair.duoBonus, spreads: fair.spreads, autofillCounts: fair.autofillCounts, botSynergy: fair.botSynergy, draft: fair.draft, winProb: fair.winProb,
    duos: duos.map(d => [d.a, d.b, `${d.shared}/5 pre-game games together`, d.team === me.team ? 'ally' : 'enemy']),
    players: rows.map(r => ({ n: r.name, team: r.team === 100 ? 'blue' : 'red', rank: r.rank, pos: r.pos, champ: r.champ, kda: r.kda, dmg: r.dmg, cs: r.cs, cspm: +(r.cs / mins).toFixed(1), ga: r.ga.ga, form: `${r.ga.wins}W-${r.ga.n - r.ga.wins}L`, streak: r.ga.streak, wr: r.ga.wr, seasonGames: r.ga.seasonGames, duo: duoNames.has(r.name), duoWith: duoWith[r.name] || null, duoShared: duoShared[r.name] ?? null, duoRecord: duoRec[r.name] || null, deniedChamp: r.ga.deniedChamp || null, masteryPts: r.ga.masteryPts || 0, flags: [!r.ga.smurf && r.ga.autofill && 'autofill', r.ga.otp && 'otp', !r.ga.otp && r.ga.masteryPts >= 150000 && 'mastery', r.ga.otpDenied && 'otp-denied', !r.ga.smurf && r.ga.tilt && 'tilt', r.ga.rusty && 'rusty', r.ga.smurf && 'smurf', r.ga.afkRisk && 'afk-risk'].filter(Boolean), place: r.place || null, badge: r.badge || null, perfScore: r.perfScore ?? null })),
  };
}
...
export async function analyzeLive(c, db, { name, tag, puuid }) {
  ...
  const entry = {
    ...
    players: rows.map(r => ({ n: r.name, team: r.team === 100 ? 'blue' : 'red', rank: r.rank, pos: r.pos, champ: r.champ, ga: r.ga ? r.ga.ga : null, form: r.ga ? `${r.ga.wins}W-${r.ga.n - r.ga.wins}L` : null, streak: r.ga ? r.ga.streak : null, wr: r.ga ? r.ga.wr : null, seasonGames: r.ga ? r.ga.seasonGames : null, duo: duoNames.has(r.name), duoWith: duoWith[r.name] || null, duoShared: duoShared[r.name] ?? null, duoRecord: duoRec[r.name] || null, deniedChamp: r.ga ? (r.ga.deniedChamp || null) : null, masteryPts: r.ga ? (r.ga.masteryPts || 0) : 0, flags: r.ga ? [!r.ga.smurf && r.ga.autofill && 'autofill', r.ga.otp && 'otp', !r.ga.otp && r.ga.masteryPts >= 150000 && 'mastery', r.ga.otpDenied && 'otp-denied', !r.ga.smurf && r.ga.tilt && 'tilt', r.ga.rusty && 'rusty', r.ga.smurf && 'smurf', r.ga.afkRisk && 'afk-risk'].filter(Boolean) : [], place: null, badge: null, perfScore: null, posInferred: true })),
  };
