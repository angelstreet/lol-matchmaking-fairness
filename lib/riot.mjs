...
const riskOf = r => (r.ga?.autofill ? 5 : 0) + (r.ga?.firstTime ? 7 : 0);
...
function fairness(rows, duos, userTeam) {
  ...
  const reasons = [];
  // Which side each triggered reason actually disadvantages, relative to the user's team — feeds
  // the overall `direction` field below ('against' / 'favor' / 'mixed'). Most reasons are already
  // scoped or worded to only ever fire against the user (duo synergy, first-timers, risk
  // asymmetry); the rest (tier spread, GA gap, heavy lanes, blowout lane, net lane) can go either
  // way and are classified per-trigger as they fire.
  let anyAgainst = false, anyFavor = false;
  if (maxSpread >= 2.5) {
    reasons.push(`huge tier spread inside a team (${maxSpread.toFixed(1)} tiers)`);
    const userSpread = userTeam === 100 ? sB : sR, enemySpread = userTeam === 100 ? sR : sB;
    if (userSpread > enemySpread) anyAgainst = true; else if (enemySpread > userSpread) anyFavor = true;
  }
  if (gaGap != null && gaGap >= 12) {
    const against = userEffGa === Math.min(effB, effR);
    reasons.push(`team strength gap (GA ${Math.round(effB)} vs ${Math.round(effR)})${against ? ' against you' : ' in your favor'}`);
    if (against) anyAgainst = true; else anyFavor = true;
  }
  const allyBonus = userTeam === 100 ? bonusB : bonusR, enemyBonus = userTeam === 100 ? bonusR : bonusB;
  const duoEdge = enemyBonus - allyBonus;
  if (duoEdge > 0) { reasons.push(`enemy duo synergy (+${duoEdge} GA) tips the balance`); anyAgainst = true; }
  const firstTimers = rows.filter(r => r.team === userTeam && r.ga?.firstTime).length;
  if (firstTimers >= 2) { reasons.push(`${firstTimers} first-time champions on your team`); anyAgainst = true; }
  ...
  if (Math.abs(riskB - riskR) >= 8 && userRisk > enemyRisk) { reasons.push('off-role/first-time picks stack the risk against you'); anyAgainst = true; }
  ...
  if (heavyDiff >= 2) {
    const favorsUser = userHeavy > enemyHeavy;
    reasons.push(`${heavyDiff} lanes heavily favored ${direction(favorsUser)}`);
    if (favorsUser) anyFavor = true; else anyAgainst = true;
  }

  const blowout = laneAdj.find(v => Math.abs(v) >= 30);
  if (blowout != null) {
    reasons.push(`a lane is massively mismatched (Δ${Math.abs(Math.round(blowout))} GA)`);
    const favorsUser = userTeam === 100 ? blowout > 0 : blowout < 0;
    if (favorsUser) anyFavor = true; else anyAgainst = true;
  }

  const netLane = laneAdj.reduce((s, v) => s + v, 0);
  const userNet = userTeam === 100 ? netLane : -netLane;
  if (Math.abs(netLane) >= 35) {
    const favorsUser = userNet > 0;
    reasons.push(`net lane advantage of ${Math.abs(Math.round(netLane))} GA ${direction(favorsUser)}`);
    if (favorsUser) anyFavor = true; else anyAgainst = true;
  }

  const verdict = reasons.length === 0 ? 'FAIR' : 'NOT FAIR';
  const oneLiner = verdict === 'FAIR' ? 'Fair lobby — result came down to play.' : reasons.join('; ') + '.';
  // 'against' = every triggered reason disadvantages the user's team, 'favor' = every one
  // advantages it, 'mixed' = both (or, defensively, neither — shouldn't happen but a NOT FAIR
  // verdict with no classified direction reads the same as 'mixed' on the frontend). null when
  // FAIR, since there's nothing to point in any direction.
  const verdictDirection = verdict === 'FAIR' ? null : (anyAgainst && anyFavor) ? 'mixed' : anyAgainst ? 'against' : anyFavor ? 'favor' : 'mixed';
  return {
    verdict, oneLiner, direction: verdictDirection, teamGA: { blue: gaB && Math.round(gaB), red: gaR && Math.round(gaR) },
...
export async function analyzeMatch(c, db, { name, tag, puuid, matchId }) {
  ...
  return {
    ...
    matchmaking: fair.verdict, direction: fair.direction, oneLiner: fair.oneLiner, teamGA: fair.teamGA, duoBonus: fair.duoBonus, spreads: fair.spreads,
