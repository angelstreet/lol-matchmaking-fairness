...
function fairness(rows, duos, userTeam) {
  ...
  if (reasons.length === 0) {
    ...
    // v4.24: when a second aligned reason fired, name it too (top two by weight) so a comparably-
    // sized second factor isn't invisible in the summary just because one reason edged it out —
    // falls back to the single-reason oneLiner above when there's no second reason, or the combined
    // sentence would run past ONE_LINER_MAX.
    if (dominant && sortedAligned[1]) {
      const combined = combinedOneLiner(dominant, sortedAligned[1], verdictDirection, effB, effR);
