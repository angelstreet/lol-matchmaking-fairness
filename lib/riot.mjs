...
export async function analyzeMatch(c, db, { name, tag, puuid, matchId }) {
  ...
  return {
    ...
    players: rows.map(r => ({ n: r.name, team: r.team === 100 ? 'blue' : 'red', rank: r.rank, pos: r.pos, champ: r.champ, kda: r.kda, dmg: r.dmg, cs: r.cs, cspm: +(r.cs / mins).toFixed(1), ga: r.ga.ga, form: `${r.ga.wins}W/${r.ga.n} ${r.ga.streak}`, streak: r.ga.streak, duo: duoNames.has(r.name), flags: [r.ga.autofill && 'autofill', r.ga.firstTime && 'first-time', r.ga.otp && 'otp'].filter(Boolean), place: r.place || null, badge: r.badge || null })),
  };
}
