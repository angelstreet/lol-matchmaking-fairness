...
// verdicts from reading EVEN when one side is carrying risk the other isn't. v4 (backtest-driven):
// first-time was -5, but first-timers in the backtest averaged place 8.8/10 — clearly
// underweighted. Bumped to -7 (autofill stays -5, it wasn't flagged), then -10 in v4.2 (a fresh
// first-time Malphite got run over in a real game that prompted this).
// v4.5: the first-time penalty is REMOVED entirely — fresh evidence at this elo (multi-season
// Diamond) is a first-time Sylas going 15/1/3 MVP and a first-time Seraphine going 1/8/27 MVP the
// same day; a champ a strong player hasn't logged recently isn't a handicap, it reads more like a
// lure. The v3 audit's 8.8/10 average place was a lower-elo mix — this is elo-contextual, flagged
// here for re-evaluation in the next audit rather than silently forgotten. "first-time" is now
// purely informational, same treatment as "tilt?" below — the chip stays (still smurf-suppressed,
// still meaningless on a fresh account), it just costs nothing. The smurf exemption below is now
// only about autofill (there's nothing left to exempt first-time FROM).
...
// this GA-affecting lane-math penalty is scaled.
...
// v4.4: a duo that INCLUDES a jungler bleeds harder than most — a jungler can path/gank on call
// for their duo partner in a way a same-lane duo can't replicate for a third teammate, and the
// jungler's own lane benefits symmetrically (partner calls enemy positioning/wards back). Applies
// to BOTH members of a jungle-inclusive duo, not just the jungler.
const JUNGLE_DUO_LANE_BONUS = 5;

// Shared by fairness()'s laneAdj and laneRecommendation() below: maps each duo'd player's name to
// the lane bonus their pair earns (JUNGLE_DUO_LANE_BONUS if either half of the pair is jungling
// this game, else the flat LANE_DUO_BONUS). A player who somehow shows up in more than one
// detected pair (rare) gets the larger of the two — same "don't undercount coordination" spirit
// as everywhere else duo bonuses are computed.
...
function fairness(rows, duos, userTeam) {
  ...
  // v4.5: the "2+ first-timers" team-level trigger is removed along with the first-time penalty
  // itself (see riskOf above) — first-time no longer costs any score, so it shouldn't cost a
  // fairness trigger either. Asymmetric risk load below now reduces to autofill only, independent
  // of the GA-gap check above (which uses raw GA and can still read "close" even when the
  // underlying picks are lopsided). Only fires against the user, matching the binary verdict's
  // framing of "was this fair to you".
  ...
  if (Math.abs(riskB - riskR) >= 8 && userRisk > enemyRisk) reasons.push({ type: 'risk', terse: 'autofill risk against you', weight: Math.abs(riskB - riskR), favorsUser: false, note: 'risk imbalance' });
  ...
  // to whichever individual lane each half of the duo is actually playing. v4.4: that bonus is
  // now jungle-aware (duoLaneBonusMap above) — a jungle-inclusive duo bleeds +5 into both lanes
  // instead of the flat +3.
  ...
  const laneAdj = ['TOP', 'JUNGLE', 'MIDDLE', 'BOTTOM', 'UTILITY'].map(pos => {
    ...
    if (!b || !r) return null;
    const bGa = b.ga.ga - riskOf(b) - roleCounterPenalty(b.champ, r.champ, pos) + (laneBonusOf[b.name] || 0);
    const rGa = r.ga.ga - riskOf(r) - roleCounterPenalty(r.champ, b.champ, pos) + (laneBonusOf[r.name] || 0);
  ...
  const net = 0.5 * netLaneUser + gaGapUser + 1.5 * duoDiff;
