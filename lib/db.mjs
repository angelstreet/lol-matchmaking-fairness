...
// Paged history of analyzed games for one summoner, newest first.
export async function listAnalyses(summoner, limit = 10, offset = 0) {
  const r = await db().execute({
    sql: `SELECT json FROM analyses WHERE summoner = ? ORDER BY json_extract(json, '$.when') DESC LIMIT ? OFFSET ?`,
    args: [summoner, limit, offset],
  });
  return r.rows.map(row => JSON.parse(row.json));
}
export async function countAnalyses(summoner) {
  const r = await db().execute({ sql: `SELECT COUNT(*) AS c FROM analyses WHERE summoner = ?`, args: [summoner] });
